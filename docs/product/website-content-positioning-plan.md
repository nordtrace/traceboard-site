...
| `resources.html` Resources | Visitors seeking proof | Route to comparison, AI, case study | Learn the product’s approach before buying | AI review model, comparison, internal case study | Links to deeper evidence | “All-in-one” if it implies arbitrary ALM scope; fully offline zero-AI claim | Resource descriptions and case-study generalization | Add Engineering State/baseline resource link if existing page capacity allows | Open resource / book walkthrough |
| `case-study.html` Case Study | Technical and compliance reviewers | Demonstrate architecture through one internal test scenario | Propose/review boundaries helped expose a specific import traceability defect | Source-ID preservation, deterministic versus LLM information, human checkpoint | AI import, Audit Narrative, provenance, source IDs, review | Any implication that all traceability bugs are automatically caught | “Real test” scope, version/date, model names, historical defect status | Add scenario scope and explicit non-guarantee note | Read AI approach / request walkthrough |
