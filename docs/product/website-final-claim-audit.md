...
| `case-study.html` | internal test | a real scenario caught a silent traceability bug using two LLMs | AMBIGUOUS | Result is reproducible/general and defect detection is guaranteed | Page bounds it as internal scenario and not a guarantee; external audit cannot independently verify the event | Keep as bounded case study; add date/version/context if publicly supportable |
| `case-study.html` | compliance/ISO 26262 discussion | scenario uses compliance language and ISO 26262 semantics | SUPPORTED — NEEDS PRECISION | Case study demonstrates compliance or ISO certification value | Inventory supports standards-aware analysis, not certification | Keep context; explicitly avoid implying compliance outcome |
